...
| `M7-WIN-005` | **【已废弃】禁止提供 ImTip 入口、探测、启动、官网跳转或任何关联集成** | P3 | `dict.aardio:8, 47-78` | 仅保留 ID 与原始来源用于审计，项目及后续阶段均不实现 |
...
### `M7-WIN-005` 已废弃：禁止 ImTip 集成

2026-08-25 用户明确决定：整个项目及全部后续阶段均不需要 ImTip。新项目不得创建相关页面、按钮、动作、托盘项、快捷键、配置键、Tauri command/capability、进程探测、可执行文件启动、URL 或依赖。`wubi-lex/` 中的原始实现只作为历史分析快照，不构成可迁移需求。
...
| ID | 项 | 文件 | 行数 | 判据 |
|---|---|---|---:|---|
| `M7-DEPR-001` | 超级热键配置模板 | `lib/app/hotkey.aardio` | 303 | 全文为注释；未被任何文件 `import`；`README.md:27` 明示「新版不再提供超级热键功能」 |
...
| `WIN` | `main.aardio:6-16`（窗体）、`37-68`（**一级 Tab**）、`71-93`（最小化/关闭/销毁）；`dlg/dict/dict.aardio:14-45`（词库二级 Tab）；`dlg/help/help.aardio:14-43`（帮助二级 Tab） | `dict.aardio:8, 47-78` 的 ImTip 入口对应已废弃的 `M7-WIN-005`，禁止迁移 |
