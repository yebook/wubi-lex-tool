...
- Native main-window and owned tray effects live in `src-tauri/src/window/`: `bounds.rs` owns pure monitor correction, `persistence.rs` owns coalesced placement writes, and `tray.rs` owns the fixed tray/menu resources. `src-tauri/src/commands/app/window.rs` remains a thin generated-IPC adapter; React must not duplicate native lifecycle state.
