...
改章节标题会静默打断**所有**指向它的 `](#...)` 链接。GitHub 不会报错，只会跳到页顶——读者以为是自己点错了。

2026-08-21 全库首次锚点扫描查出 **7 处既有断链**，成因全是章节改名或改号后没回改引用：

| 断链 | 成因 |
|---|---|
| `21-ui-ux.md#占位状态规范ux-interact-013` ×2 | 21 整体重写后该节改名为 `6.1 占位状态规范` |
| `21-ui-ux.md#原项目配色参考` | 同上，该节被删除 |
| `M7-app-shell.md#7-已废弃模块p3` | 章节由 §7 改号为 §8 |
| `22-roadmap.md#阶段-s8--自学习` | 标题带 `→ v1.1` 后缀，slug 尾部多了 `--v11` |
| `M5-etymon-help.md#附录-b微软五笔使用技巧-wubiaardio19-39` | 标题里的反引号被 slug 吃掉，多写了一个连字符 |
| `03-source-index.md#libapphotkeyaardiop3` | 指向一个从未存在的章节 |

### 校验脚本

```bash
python .trellis/scripts/check_anchors.py   # 期望输出「0 处失效」
```

### 写这个脚本时踩的两个坑

**坑 1：GitHub 的 slugger 不 trim 首尾空白。**

```
### ⚠️ 性能红线   →  #-性能红线      （emoji 被删，残留的空格变成前导连字符）
## `lib/wubi/lexFile.aardio` ★  →  #libwubilexfileaardio-   （尾部同理）
```

脚本里顺手加个 `.strip()` 会把这类**正确**的链接误报成断链——首轮扫描的 39 条里有 9 条是这么来的假阳性。

**坑 2：相对路径要 `os.path.normpath`。**

`docs/modules/M1-lex-table.md` 里的 `../01-data-formats.md` 不做规范化就成了 `docs/modules/../01-data-formats.md`，与实际文件名对不上——首轮 39 条里另有 8 条假阳性出自这里。

> 两个坑叠加，39 条报告里有 17 条是脚本自己的问题。**校验脚本本身也要被验证**：先手工确认几条「失效」是真的，再信任它的输出。
